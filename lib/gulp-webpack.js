...
module.exports = function(settings){
  return gulp.task("dev", function() {
    ...
    new WebpackDevServer(webpack(myConfig), {
      ...
      logger.info('server start on localhost:8080')
      let url;
      dir.split(',').forEach(function (v,i) {
        if(v=='game'){
          url = 'http://localhost:8080/www/build/html/game.html?code=ssss#zhida'
        }
        else {
          url = 'http://localhost:8080/www/build/html/' + v + '.html'
        }
        shell.exec('open ' + url)
      })
