...
module.exports = {
  ...
  envSet(){
    var me = this
    me.dev = me.settings.env == 'dev' ? true : false
    me.prod = !me.dev
  },
  output: function () {
      ...
      if (v !== 'script' && fs.lstatSync(temp).isDirectory()) {
        fs.readdirSync(temp).forEach(function (value, index) {
          if (path.extname(value) == '.js') {
            ...
            me.buildDir.push({dir:v,file:key})
          }
  ...
  htmlPluginsConfig(){
    ...
    me.buildDir.forEach((v, i) => {
      let htmlConfig = {
        cache: true,
        template: './html/' + v.dir + '.html',
        chunks: [v.file],
        inlineSource: '.(js)$',
        filename: path.resolve(settings.output, 'html', v.dir + '.html')
      }
      if (me.prod) {
        htmlConfig.minify = {
          "removeComments": true,
        ...
        config.plugins.unshift(new HtmlWebpackPlugin(htmlConfig))
      }
      else {
        config.plugins.unshift(new HtmlWebpackPlugin(htmlConfig))
      }
    ...
    htmlDir.forEach(function (v, i) {
      let flag = false
      me.buildDir.forEach(function (value) {
        if ((value.dir + '.html') == v) {
          flag = true
        }
      })
  ...
  plugin: function () {
    ...
    if (me.prod) {
      ...
      //  ModuleConcatenationPlugin & uglify
